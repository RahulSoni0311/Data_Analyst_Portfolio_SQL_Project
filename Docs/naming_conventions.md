...
- **`dwh_<column_name>`**
  ...
  - `<column_name>`: Descriptive name indicating the purpose of the column.
...
- **`load_<layer>`**
  ...
  - Examples:
    - `load_bronze` → Stored procedure for loading the Bronze layer.
    - `load_silver` → Stored procedure for loading the Silver layer.